...
#             # return redirect('payment_success',order_id=order_id, payment_id=payment_id)
#             print(f"Payment processing error: {str(e)}")
#             return redirect(f'/payment_failed/?order_id={order_id}&payment_id={payment_id}')
...
import razorpay
from django.conf import settings
from .models import Event, deccart, decorationtransaction
...
def buy_all_decorations(request):
    if request.method == 'POST':
        ...
        if payment_id and order_id:
            ...
            if payment['status'] == 'captured':
                email = request.session.get('email')
                user = Event.objects.filter(email=email).first()

                if user:
                    cart_items = deccart.objects.filter(user__email=email)
                    for cart_item in cart_items:
                        decorationtransaction.objects.create(
                            user=user,
                            razorpay_order_id=order_id,
                            razorpay_payment_id=payment_id,
                            amount=cart_item.amount,
                    ...
                    # Redirect to success page without order_id or payment_id
                    return redirect('payment_success1')
...
def payment_failed(request):
    return render(request, 'payment_failed.html', {'message': 'Payment failed. Try again.'})
...
def approve_refund(request, refund_id):
    ...
    refun = refund.objects.get(id=refund_id)
    tbo=refun.transact.id
    refunds = Payment.objects.get(id=tbo)
...
def payment_success1(request, order_id, payment_id):
    # Initialize Razorpay client
    client = razorpay.Client(auth=(settings.RAZOR_KEY_ID, settings.RAZOR_KEY_SECRET))
